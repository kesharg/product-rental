...
class ProductController extends Controller
{
    /*public function show(Request $request, $id)
    {
        try {
            ...
            // Eager load the pricing, applying the filters for region_id and rental_period_id
            if ($request->has('region_id') || $request->has('rental_period_id')) {
                $product->load(['pricing' => function ($query) use ($request) {
                    // Apply region filter if it exists
                    if ($request->has('region_id')) {
                        $query->where('region_id', $request->region_id);
                    }

                    // Apply rental_period filter if it exists
                    if ($request->has('rental_period_id')) {
                        $query->where('rental_period_id', $request->rental_period_id);
                    }

                    // Eager load the related region and rental period data
                    $query->with(['region', 'rentalPeriod']);
                }]);
            } else {
                $product->load(['pricing' => function ($query) {
                    // Load all pricing without filters if no region_id or rental_period_id are applied
                    $query->with(['region', 'rentalPeriod']);
                }]);
            }
